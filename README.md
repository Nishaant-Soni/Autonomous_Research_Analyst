...
- **Eval Score stage (Phase 4 Group B1 + B2 complete)** — `python -m eval.score` reads cached artifacts from a run dir and writes `scores.json` with deterministic citation accuracy (from the citation validator's `stripped_fraction`) and Ragas-judged faithfulness, answer relevancy, and context recall, plus the derived headline `hallucination_rate = 1 − faithfulness` (PRD §10). The Ragas judge runs on a separate `langchain_openai.ChatOpenAI(gpt-5.4-mini)` instance, keeping eval infrastructure out of the runtime path. Failed items are surfaced separately rather than averaged in as zero. `--skip-ragas` keeps the deterministic-only path available when the eval extra isn't installed.

Not yet built (Phase 4 Group B3 + Group C, plus Phase 5): latency/cost from LangSmith, the versioned aggregate eval report, the embedding A/B, prompt-tuning before/after, and the UI.
