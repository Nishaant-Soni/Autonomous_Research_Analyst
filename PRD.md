...
- API write endpoints are rate-limited (per authenticated user; per IP for the auth endpoints) to bound abuse and runaway cost; read-only polling endpoints are exempt so the live progress view isn't throttled.
- **Prompt injection — containment, not prevention.** Web *and* user-document content is treated as untrusted: snippets are fenced in untrusted-data markers (with anti-spoofing) and both the Researcher and Writer are instructed to treat fenced content as data, never instructions. Injection can't be fully prevented, so blast radius is bounded instead — read-only tools (no code execution, no arbitrary URLs/SSRF, no destructive DB actions), fail-closed per-user RAG scoping, the citation validator dropping any claim whose citation doesn't resolve, and safe Markdown rendering (no raw HTML, sanitized link schemes) on the report.
